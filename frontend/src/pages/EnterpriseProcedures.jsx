...
import {
  ClipboardList, Upload, File, Trash2, RefreshCw, Plus, X,
  ...
  const download = async () => {
    ...
    try {
      const res = await api.get(
        `/enterprise-procedures/libraries/${libraryId}/documents/${doc.id}/download`,
        { responseType: 'blob' }
      )
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          onClick={() => openCyberAssistant({
            ...
            attachments: [
              { type: 'library', resource_id: String(libraryId), context_json: { library_kind: 'procedure' } },
              {
                type: 'document',
        ...
        {isActive && !['processing'].includes(doc.parse_status) && <span className="text-xs text-gray-400 italic px-1">In queue</span>}
        {doc.parse_status === 'indexed' && (
...
function LibraryCard({ library, onDeleted }) {
  ...
  const { data: docs = [], refetch } = useQuery({
    queryKey: ['proc-docs', library.id],
    queryFn: () => api.get(`/enterprise-procedures/libraries/${library.id}/documents`).then(r => r.data),
    enabled: open,
  ...
  const { data: processingProgress } = useQuery({
    queryKey: ['proc-progress', library.id],
    queryFn: () => api.get(`/enterprise-procedures/libraries/${library.id}/processing-progress`).then(r => r.data),
    enabled: open && hasProcessing,
    refetchInterval: hasProcessing ? 1000 : false,
  })

  const deleteMut = useMutation({
    mutationFn: (docId) => api.delete(`/enterprise-procedures/libraries/${library.id}/documents/${docId}`),
    onSuccess: () => { refetch(); qc.invalidateQueries(['proc-libraries']) },
  })
  ...
  const uploadFiles = async (files) => {
    ...
    for (const file of files) {
      ...
      try {
        await api.post(`/enterprise-procedures/libraries/${library.id}/documents`, form)
      } catch (e) {
    ...
    refetch()
    qc.invalidateQueries(['proc-libraries'])
    setUploading(false)
  }

  const reparse = async (docId) => {
    try {
      await api.post(`/enterprise-procedures/libraries/${library.id}/documents/${docId}/reparse`)
      refetch()
  ...
  const reindex = async (docId) => {
    try {
      await api.post(`/enterprise-procedures/libraries/${library.id}/documents/${docId}/reindex`)
      refetch()
    } catch (e) { alert('Reindex failed: ' + (e?.response?.data?.detail || e?.message)) }
  }

  const indexed = docs.filter(d => d.parse_status === 'indexed').length
  const active  = docs.filter(d => ['pending', 'processing', 'indexing', 'queued'].includes(d.parse_status)).length
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mb-4">
      <div
        ...
        onClick={() => setOpen(o => !o)}
      >
        <ClipboardList size={18} className="text-teal-600 flex-shrink-0" />
        <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            onClick={(e) => {
              ...
              openCyberAssistant({
                mode: 'workspace',
                title: `${library.name} Procedure Assistant`,
                attachments: [
                  { type: 'library', resource_id: String(library.id), context_json: { library_kind: 'procedure' } },
                ],
              })
            }}
            className="p-1.5 text-violet-500 hover:text-violet-700 transition-colors rounded"
            title="Ask AI about this procedure library"
          >
                ...
                <div className="flex-1 mx-3 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      indexed === docs.length ? 'bg-emerald-500' : active > 0 ? 'bg-teal-400' : 'bg-amber-400'
                    }`}
...
function NewLibraryModal({ onClose, onCreate }) {
  ...
  const save = async () => {
    ...
    setError('')
    try {
      await api.post('/enterprise-procedures/libraries', { ...form, category: 'general' })
      onCreate()
      onClose()
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-gray-900">New Procedure Library</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text" value={form.name} placeholder="e.g. Access Control Procedures"
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={save} disabled={saving}
            className="flex-1 bg-teal-600 text-white rounded-lg py-2 text-sm hover:bg-teal-700 disabled:opacity-50">
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function EnterpriseProcedures() {
  const qc = useQueryClient()
  const [showModal, setShowModal] = useState(false)

  const { data: libraries = [], isLoading } = useQuery({
    queryKey: ['proc-libraries'],
    queryFn: () => api.get('/enterprise-procedures/libraries').then(r => r.data),
  })

  const deleteMut = useMutation({
    mutationFn: (id) => api.delete(`/enterprise-procedures/libraries/${id}`),
    onSuccess: () => qc.invalidateQueries(['proc-libraries']),
  })
    ...
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Organization-wide procedure documents. Uploaded documents are parsed, chunked, and indexed
            against NIST 800-53 controls. Click any indexed filename to explore the evidence.
          </p>
        </div>
        <button
          onClick={() => openCyberAssistant({
            mode: 'workspace',
            title: 'Enterprise Procedure Assistant',
            attachments: [{ type: 'general', resource_id: 'general', context_json: { label: 'Enterprise Procedure Library' } }],
          })}
        ...
        <button
          ...
          className="flex items-center gap-2 bg-teal-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-teal-700 transition-colors shadow-sm flex-shrink-0 ml-4"
        >
          <Plus size={16} />New Library
        </button>
      </div>

      <div className="flex items-start gap-3 bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 mb-6 text-sm text-teal-800">
        <FolderOpen size={16} className="text-teal-500 flex-shrink-0 mt-0.5" />
        <p>
          Procedure documents are parsed, chunked, and indexed against NIST 800-53 Rev 5 controls at upload time.
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">No procedure libraries yet.</p>
          <p className="text-xs mt-1">Create a library to start uploading procedure documents.</p>
        </div>
      ) : (
        libraries.map(lib => (
          <LibraryCard key={lib.id} library={lib} onDeleted={(id) => deleteMut.mutate(id)} />
        ))
      )}
      ...
      {showModal && (
        <NewLibraryModal
          ...
          onCreate={() => qc.invalidateQueries(['proc-libraries'])}
        />
      )}
    </div>
  )
